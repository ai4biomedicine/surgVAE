...
class VAE_attention(nn.Module):
    ...
    def __init__(self, z_dim=10, input_dim=663):
        ...
        self.classifiers = nn.ModuleList(
            [nn.Sequential(
            nn.Dropout(0.2),
            nn.Linear(2 * z_dim, 8192),
            nn.ReLU(True),
            nn.Dropout(0.5),
            nn.Linear(8192, 4096),
            nn.ReLU(True),
            nn.Dropout(0.5),
            nn.Linear(4096, 3) #binary classification with one dummy class
        ) for i in range(6)])
    ...
    def forward(self, x):
        ...
        classifications =  torch.cat([classifier(stats).unsqueeze(1) for classifier in self.classifiers], dim=1)
        

        return x_recon, mu, logvar, z.squeeze(), classifications
